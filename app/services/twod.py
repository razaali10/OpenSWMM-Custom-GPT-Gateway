...
async def get_twod_summary(client: MCPClient, session_id: str) -> dict:
    mesh = await client.call_tool("twod_get_mesh_summary", {"session_id": session_id})
    if not mesh.get("active"):
        return {"session_id": session_id, "active": False, "mesh": None, "totals": None}
    totals = await client.call_tool("twod_get_totals", {"session_id": session_id})
...
async def get_twod_mass_balance(client: MCPClient, session_id: str) -> dict:
    ...
    balance = await client.call_tool("twod_get_mass_balance", {"session_id": session_id})
...
async def get_twod_coupling(client: MCPClient, session_id: str) -> dict:
    ...
    coupling = await client.call_tool("twod_get_coupling_map", {"session_id": session_id})
